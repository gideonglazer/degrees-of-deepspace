...
(function () {
	...
	const S = C().school;
	const FRIDAY_GUESTS = S.fridayGuests;
	const GATE_OPEN = S.gateOpen;
	...
	const DEFAULT_LETTERS = S.defaultLetters;
	const DEFAULT_UNDERSTANDING = S.defaultUnderstanding;
	const STAR_UNDERSTANDING = S.starUnderstanding;
	const MAX_DAILY_STARS = S.maxDailyStars;
	const TURNS_PER_CLASS = S.turnsPerClass;
	const TURNS_PER_EXAM_CLASS = S.turnsPerExamClass;
	const FOCUS_UNDERSTANDING = S.focusUnderstanding;
	const STUDY_MINUTES = S.studyMinutes;
	const STUDY_UNDERSTANDING = S.studyUnderstanding;
	const STUDY_BEFORE_SUBJECTS = S.studyBeforeSubjects;
	const FIRST_STUDY_WINDOW = S.firstStudyWindow;
	const STUDY_BEFORE_WINDOW = S.studyBeforeWindow;
	const STUDY_BEFORE_LABELS = S.studyBeforeLabels;
	const STUDY_BEFORE_GENERAL = S.studyBeforeGeneral;
	const STUDY_BEFORE_BY_GRADE = S.studyBeforeByGrade;
	const UNDERSTANDING_MIN = S.understandingMin;
	const UNDERSTANDING_MAX = S.understandingMax;
	const DEMOTE_AT = S.demoteAt;
	const SUNDAY_PROMOTE_AT = S.sundayPromoteAt;
	const WEEKLY_DECAY = S.weeklyDecay;
	const SKILLS = S.skills;
	const QUALITY_CLASSES = S.qualityClasses;
	const QUALITY_LABELS = S.qualityLabels;
	const GRADE_COMMENTS = S.gradeComments;
	const ROOMS = S.rooms;
	const EARLY_COPY = S.earlyCopy;
	const FOCUS_GRADE_BONUS = S.focusGradeBonus;
	...
	function getDailyProgress(subject, variables) {
		...
		if (!subject || !(subject in DEFAULT_LETTERS)) return 0;
		const day = vars.school.dailyProgress[World.dayKey(vars)];
	...
	function hasAttended(periodKey, variables) {
		...
		const day = vars.school.attended[World.dayKey(vars)];
	...
	function getPeriodTurns(periodKey, variables) {
		...
		const day = vars.school.periodTurns[World.dayKey(vars)];
	...
	function formatActionMinutes(minutes) {
		return World.formatActionMinutes(minutes);
	}

	/**
	 * Minutes from now until this room's next period starts (at least 1).
	 */
	function minutesUntilPeriodStart(roomKey, variables) {
		const vars = variables || V();
	...
	function studyBeforeMinutes(roomKey, variables) {
		return minutesUntilPeriodStart(roomKey, variables);
	}
