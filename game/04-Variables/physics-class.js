...
(function () {
	...
	/** Base chance that Focus diverts into an interactive vignette. */
	const FOCUS_CHANCE = 25;
	...
	function weatherKey(variables) {
		return World.ensure(variables).weather || "fair";
	}
	...
	function eligibleDuring(variables) {
		const weather = weatherKey(variables);
	...
	function resolveHonorific(text, variables) {
		const raw = String(text || "");
		const honorific = typeof Pronouns !== "undefined" && Pronouns.honorific
			? Pronouns.honorific(variables)
			: "Mx.";
		return raw.split("{honorific}").join(honorific);
	}

	/**
	 * Preview/apply markup for an extra Physics daily star (study-before style).
	 */
	function bonusStarMarkup(variables, appliedLevel) {
		const vars = variables || V();
		const current = School.getDailyProgress(ROOM_KEY, vars);
		...
		return School.dailyStarEffectMarkup(ROOM_KEY, level);
	}
	...
	function findFocusEntry(id) {
		return FOCUS.find(entry => entry.id === id) || null;
	}

	/**
	 * Picks a during-class vignette for the current weather and stores display strings.
	 */
	function rollDuring(variables) {
		const vars = variables || V();
		const pool = eligibleDuring(vars);
		const pick = pickRandomItemInArray(pool.length ? pool : DURING);
		const effectsMarkup = Stats.applyEffects(pick.effects, vars);
		vars.physicsDuringText = pick.text;
		vars.physicsDuringEffects = effectsMarkup || "";
		return {
			id: pick.id,
			text: pick.text,
			effectsMarkup,
	...
	/**
	 * Picks an end-of-class vignette and stores it on $physicsEnd.
	 */
	function rollEnd(variables) {
		...
		return vars.physicsEnd;
	}

	/**
	 * True once today's physics period is attended and the end vignette hasn't played yet.
	 */
	function needsEnd(variables) {
		const vars = variables || V();
		const key = periodKey(vars);
		if (!key || !School.hasAttended(key, vars)) return false;
		return vars.physicsEndDone !== dayKey(vars);
	}

	function markEndDone(variables) {
		const vars = variables || V();
		vars.physicsEndDone = dayKey(vars);
		return vars.physicsEndDone;
	}

	function clearFocusEvent(variables) {
		const vars = variables || V();
		delete vars.physicsFocus;
		delete vars.physicsFocusResult;
	}

	/**
	 * Run a normal Focus turn, then maybe divert into an interactive Focus vignette.
	 */
	function focus(variables) {
		...
		if (!FOCUS.length || getRandomIntInclusive(1, 100) > FOCUS_CHANCE) {
			return { event: false };
		}
		const pick = pickRandomItemInArray(FOCUS);
		vars.physicsFocus = {
			id: pick.id,
			text: resolveHonorific(pick.text, vars),
	...
	/**
	 * Resolve a Focus vignette choice into $physicsFocusResult.
	 */
	function applyFocusChoice(choiceId, variables) {
		const vars = variables || V();
		const active = vars.physicsFocus;
		...
		let starMarkup = "";
		let stars = School.getDailyProgress(ROOM_KEY, vars);
		if (outcome.bonusStar && stars < School.MAX_DAILY_STARS) {
			stars = School.addDailyProgress(ROOM_KEY, 1, vars);
			starMarkup = bonusStarMarkup(vars, stars);
		}

		vars.physicsFocusResult = {
			id: entry.id,
			choiceId: choice.id,
			text: resolveHonorific(outcome.result, vars),
			effectsMarkup: (starMarkup || "") + (effectsMarkup || ""),
			rolled,
		};
		delete vars.physicsFocus;
		return vars.physicsFocusResult;
	}

	Object.assign(PhysicsClass, {
		FIRST_FLAG,
		isInSession,
		rollDuring,
		rollEnd,
