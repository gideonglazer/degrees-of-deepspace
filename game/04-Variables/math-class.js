...
(function () {
	...
	const ROOM_KEY = "math";
	const FIRST_FLAG = "mathFirstClass";
	...
	/**
	 * Preview/apply markup for an extra Math daily star (study-before style).
	 */
	...
	/**
	 * Picks a during-class vignette and stores display strings.
	 */
	function rollDuring(variables) {
		...
		vars.mathDuringText = pick.text;
		vars.mathDuringEffects = effectsMarkup || "";
		return {
			id: pick.id,
			text: pick.text,
			effectsMarkup,
	...
	/**
	 * Picks an end-of-class vignette and stores it on $mathEnd.
	 */
	...
	/**
	 * True once today's math period is attended and the end vignette hasn't played yet.
	 */
	function needsEnd(variables) {
		const vars = variables || V();
		const key = periodKey(vars);
		if (!key || !School.hasAttended(key, vars)) return false;
		return vars.mathEndDone !== dayKey(vars);
	}

	function markEndDone(variables) {
		const vars = variables || V();
		vars.mathEndDone = dayKey(vars);
		return vars.mathEndDone;
	}

	function clearFocusEvent(variables) {
		const vars = variables || V();
		delete vars.mathFocus;
		delete vars.mathFocusResult;
	}
	...
	/**
	 * Resolve a Focus vignette choice into $mathFocusResult.
	 */
	function applyFocusChoice(choiceId, variables) {
		const vars = variables || V();
		const active = vars.mathFocus;
		...
		delete vars.mathFocus;
		return vars.mathFocusResult;
	}

	Object.assign(MathClass, {
		FIRST_FLAG,
		isInSession,
		rollDuring,
		rollEnd,
